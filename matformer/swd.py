...
def extract_patches(pyramid_layer, slice_indices,
                    slice_size=7, unfold_batch_size=128):
    ...
    # normalize along ch
    std, mean = torch.std_mean(x, dim=(1, 3, 4), keepdim=True)
    x = (x - mean) / (std + 1e-8)
    # reshape to 3rank [batch, n_descriptors, ch * slice_size * slice_size]
    x = x.reshape(x.size(0), x.size(1), -1)
    return x

def swd(image1, image2, 
        n_pyramids=None, slice_size=7, n_descriptors=128,
        n_repeat_projection=128, proj_per_repeat=4, device=None, return_by_resolution=False,
    ...
    device = device or image1.device
    ...
    with torch.no_grad():
        ...
        for i_pyramid in range(n_pyramids + 1):
            ...
            # patch : 3rank (batch, n_descriptors, slice_size**2*ch)
            p1 = extract_patches(pyramid1[i_pyramid], indices, slice_size=slice_size)
            p2 = extract_patches(pyramid2[i_pyramid], indices, slice_size=slice_size)
            ...
            for j in range(n_repeat_projection):
                # random
                rand = torch.randn(p1.size(2), proj_per_repeat).to(device)  # (slice_size**2*ch, proj_per_repeat)
                rand = rand / torch.std(rand, dim=0, keepdim=True)  # normalize
                # projection
                proj1 = torch.matmul(p1, rand)  # (batch, n_descriptors, proj_per_repeat)
                proj2 = torch.matmul(p2, rand)  # (batch, n_descriptors, proj_per_repeat)
                proj1, _ = torch.sort(proj1, dim=1)
                proj2, _ = torch.sort(proj2, dim=1)
                d = torch.abs(proj1 - proj2)
                distances.append(torch.mean(d, dim=(1, 2)))  # (batch,)

            # swd
            result.append(torch.mean(torch.stack(distances, dim=1), dim=1))  # (batch,)
        
        # average over resolution
        result = torch.stack(result, dim=1)  # (batch, n_pyramids + 1)
        if return_by_resolution:
            return result
        else:
            return torch.mean(result, dim=1)  # (batch,)
